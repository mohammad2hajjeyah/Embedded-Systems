...
char scanKeypad() {
    ...
    for (int i = 0; i < 4; i++) rows[i]->write(1);
...
char waitForKey() {
    while (scanKeypad() != 0) { thread_sleep_for(10); }
    char k = 0;
    while (k == 0) {
        k = scanKeypad();
        thread_sleep_for(20);
...
bool enterDeactivationCode() {
    ...
    for (int i = 0; i < 4; i++) {
        if (entered[i] != DEACTIVATE_CODE[i]) return false;
    }
    return true;
}
...
int main() {
    ...
    while (true) {
        ...
        if ((tempWarning || gasWarning) && !alarmActive) {
            ...
            if (tempWarning && !gasWarning) cause = "Temperature";
            else if (gasWarning && !tempWarning) cause = "Gas";
